...
import { getRevenue } from "./DashboardAPI";
import { ApexOptions } from "apexcharts";
import styles from "./FrameComponent.module.css";
import ReactApexChart from "react-apexcharts";

export default function GetRevenue({ advertId }: AdvertIdProps) {
  const accessToken = useSelector(
    (state: RootState) => state.token.accessToken
  );
  const [seriesData, setSeriesData] = useState([
    { name: "수익", data: [] }, // 초기 데이터 빈 배열로 설정
  ]);

  useEffect(() => {
    if (advertId && accessToken) {
      getRevenue(advertId, accessToken).then((apiData) => {
