...
export default function GoEnrollButton({
  children,
  to,
  formData,
}: GoEnrollButtonProps) {
  ...
  const handleClick = async () => {
    if (
      ...
      formData.endDate != "" &&
      formData.bannerImgUrl != undefined &&
      formData.bannerImgUrl != "" &&
      formData.advertVideoUrlList != undefined &&
      formData.advertVideoUrlList != null
    ) {
  ...
  const buttonClass =
    to === "/ad-enroll/merchandise"
      ? styles.enrollButton
      : styles.directAdButton;
  return (
    <div>
      <button
        className={`${styles.goButton} ${buttonClass}`}
        onClick={handleClick}
      >
        {children}
      </button>
